...
## Refreshing the diagnostics catalog

`/lookup` is backed by `src/Accord.Services/Diagnostics/diagnostics.json`, generated by the
`Accord.DiagnosticsHarvester` design-time tool. The tool is not part of the runtime build or the
Docker image. Regenerate the dataset when you bump the `Microsoft.CodeAnalysis.*` package versions
in `Directory.Packages.props`, or to add a new analyzer source:

```bash
dotnet run --project src/Accord.DiagnosticsHarvester
git add src/Accord.Services/Diagnostics/diagnostics.json
git commit -m "chore: refresh diagnostics catalog"
```

To add another analyzer family (e.g. StyleCop `SA####`, Roslynator `RCS####`): add the analyzer
package to `Directory.Packages.props`, add a matching `PackageReference` (with
`GeneratePathProperty="true" PrivateAssets="all" ExcludeAssets="analyzers"`) and a `<None ...>` copy
entry to `Accord.DiagnosticsHarvester.csproj`, then re-run the harvester.
